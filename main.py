...
def fetch_and_display_image(search_filename):
    ...
    url = f"https://{KUDU_HOST}/api/vfs/site/wwwroot/{filename}"
...
with gr.Blocks(title="Azure Image Manager") as demo:
    gr.Markdown("# 🌐 Azure App Service Image Manager")
    gr.Markdown(f"Connected to regional host: `{KUDU_HOST}`")
